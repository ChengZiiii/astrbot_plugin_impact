...
class ImpactStoreBasicMixin:
    ...
    def register_group_member(
        ...
        with self._connect() as connection:
            connection.execute(
                "INSERT INTO group_memberships(group_id, user_id, joined_at, last_active_at) VALUES (?, ?, ?, ?) "
                "ON CONFLICT(group_id, user_id) DO UPDATE SET last_active_at = excluded.last_active_at",
                (group_id, user_id, now_ts, now_ts),
            )

    def register_group_members(
        self,
        group_id: int,
        user_ids: list[int],
        initial_length: float = 10.0,
        ...
        for user_id in user_ids:
            self.register_group_member(group_id, user_id, initial_length)
    ...
    def get_group_rankings(self, group_id: int) -> list[RankEntry]:
        with self._connect() as connection:
            rows = connection.execute(
                "SELECT users.user_id, users.jj_length "
                "FROM group_memberships "
                "INNER JOIN users ON users.user_id = group_memberships.user_id "
                "WHERE group_memberships.group_id = ? "
                "ORDER BY users.jj_length DESC, users.user_id ASC",
                (group_id,),
            ).fetchall()
        return [
            RankEntry(user_id=int(row["user_id"]), length_cm=float(row["jj_length"]))
    ...
    def get_global_rank(self, user_id: int) -> int | None:
        with self._connect() as connection:
            row = connection.execute(
                "SELECT jj_length FROM users WHERE user_id = ?",
                (user_id,),
            ).fetchone()
            if row is None:
                return None
            ...
            rank_row = connection.execute(
                "SELECT COUNT(*) + 1 AS rank_no FROM users "
                "WHERE jj_length > ? OR (jj_length = ? AND user_id < ?)",
                (current_length, current_length, user_id),
            ).fetchone()
        return int(rank_row["rank_no"]) if rank_row is not None else None
